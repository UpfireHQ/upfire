...
export default class UploadsPage extends PureComponent {
  ...
  get EncryptionPopup() {
    ...
    switch (generateTorrentStatus) {
      ...
      case TS_SET_COSTS:
        return <UploadProgressPopup key="ProgressPopup" />;
    }
  ...
  get TorrentsList() {
    ...
    return torrents.map((item, i) => {
      const color = Boolean(item[TS_STATUS_DELETED])
        ? { color: DeletedTorrent }
        : {};
  ...
  render() {
    if (this.isUploadTorrent) {
      ...
      if (wallet && wallet.address) {
        return <UploadTorrent {...this.props} />;
      }
    ...
    return [
      <PageWrapper key="UploadPage">
        <PageHead>
          ...
          {/* <div className="title">{trans('Upload.title')}</div> */}
