...
export default class UploadTorrent extends React.Component {
  ...
  static get pattern() {
    return `[0-9]+(\\.[0-9]{0,${UPR_FLOAT_NUMBERS}})?%?`;
  }

  static getDerivedStateFromProps(props, state) {
    const { uploadFiles } = props;
    const size =
      uploadFiles && uploadFiles.files && prettySize(uploadFiles.files);
  ...
  handlerChangeName = e => {
    ...
    this.onChangeInput('name', e.target || {}, () => {
      return FILENAME_FILTER_SYMBOLS.split('').reduce(
        (all, symbol) => (~value.indexOf(symbol) ? false : all),
        true
      );
    });
  };
              ...
              <UInput
                ...
                error={
                  validationError && !name
                    ? trans('upload.error.torrentName')
                    : validationErrorFile
                    ? trans('upload.error.torrentNameFileName')
                    : null
                }
              />
            </HalfColumn>
            <HalfColumn>
              <FileCover>
                <UInput
                  ...
                  error={
                    validationError && !Number(cost)
                      ? trans('upload.error.torrentCost')
                      : null
                  }
                />
                <UfrInfoWrapper>
                  <Info description={trans('popups.upload.ETHInfo')} />
                </UfrInfoWrapper>
              </FileCover>
            </HalfColumn>
                ...
                <UInput
                  ...
                  error={
                    validationError && !path
                      ? trans('upload.error.torrentPath')
                      : null
                  }
                />
