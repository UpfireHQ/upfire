...
export default class DownloadsPage extends PureComponent {
  ...
  componentDidMount() {
    const { onGetPercentageOfFee } = this.props;
  ...
  handlerCheckAll = e => {
    ...
    const data = torrents.reduce(
      (res, t) => Object.assign(res, { [t.infoHash]: checked }),
      {}
    );
  ...
  get DownloadList() {
    ...
    return torrents.map((torrent, i) => {
      ...
      const color = torrent.failed
        ? { cursor: 'pointer', color: colors.defaultBtnDisabled }
        : {};
  ...
  get EmptySearch() {
    return (
      <EmptyTorrentsSearch colSpan={7} onClick={this.handlerClearSearchInput} />
    );
  }
  ...
  get Details() {
    const { onDeleteDownloadTorrent, percentageFee } = this.props;
    return (
      <Details
  ...
  render() {
    if (this.isNewDownload) {
      const {
        newDownload,
        newDownloadStatus,
        onStartDownloadTorrent,
        onCancelDownloadTorrent,
        percentageFee
      } = this.props;
      return (
        <TorrentPopup
    ...
    return (
      ...
      >
        <PageHeadDownload>
          ...
          {/* <div className="title">{trans('download.title')}</div> */}
