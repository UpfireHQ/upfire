...
export const getLatestData = async () => {
  return rp({
    uri: getLatestUrl(),
    headers: {
      'User-Agent': 'Request-Promise, Upfire'
    },
    json: true
  }).catch(e => {
...
export const getInstallerUrl = data => {
  ...
  if (process.platform === 'win32') {
    search = new RegExp('upfire(.*)\\.exe', 'i');
  } else if (process.platform === 'darwin') {
    search = new RegExp('upfire(.*)\\.dmg', 'i');
  } else if (process.platform === 'linux') {
    search = new RegExp('upfire(.*)\\.deb', 'i');
  }

  if (search && data && Array.isArray(data.assets)) {
    const asset = Array.from(data.assets).find(a =>
      search.test(String(a && a.name))
    );
