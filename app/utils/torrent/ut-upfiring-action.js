...
export class TorrentUpfiringAction extends EventEmitter {
  ...
  onExtendedHandshake(handshake) {
    if (!handshake.m || !handshake.m[TorrentUpfiringAction.EXTENSION_NAME]) {
      return this.emit(
        'warning',
        new Error('Peer does not support Torrent Upfire Action')
      );
    }
  ...
  response(type, payload) {
    const buf = bencode.encode(
      JSON.stringify({
        action: TORRENT_UPFIRING_ACTION_RESPONSE,
        type,
        payload
      })
    );
  ...
  request(type) {
    const buf = bencode.encode(
      JSON.stringify({ action: TORRENT_UPFIRING_ACTION_REQUEST, type })
    );
    try {
      this._wire.extended(TorrentUpfiringAction.EXTENSION_NAME, buf);
    } catch (e) {
