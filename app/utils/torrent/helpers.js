...
import { getETHFee } from '../../containers/Wallet/selector';
...
export const checkOwnerByWires = (torrent = {}) => {
  const owners = Array.from(
    torrent.wires ? Object.values(torrent.wires) : []
  ).reduce((value, wire) => {
  ...
  return Array.from(Object.values(torrent.wires || {}))
    .filter(
      w =>
        ...
        w.address !== owner &&
        w.downloaded &&
        Boolean(w.hasToken) === Boolean(hasToken)
    )
    .map(w => w.address);
};

export const getPaymentData = (torrent, percentageFee) => {
  ...
  const fee = getETHFee(percentageFee, amount);
  ...
  if (
    ...
    return Array.from(clientTorrent.wires).reduce((value, wire) => {
      if (
        ...
        wire.peerId &&
        storeTorrent.wires &&
        storeTorrent.wires[wire.peerId]
      ) {
