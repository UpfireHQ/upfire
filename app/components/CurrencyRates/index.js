...
export default class extends React.PureComponent {
  ...
  getRatesPromise() {
    ...
    return {
      abort: () => (aborted = true),
      promise: api
        .get('https://api.coinmarketcap.com/v2/ticker/2178/?convert=BTC')
        .then(data =>
          aborted
      ...
      currencyData && (
        <Index>
          <Head>
            <HeadRates>
              <Ufr>
                {trans('productName')} ({trans('popups.upload.ETH')})
              </Ufr>
              <Usd>
                {Number(currencyData.quotes.USD.price.toFixed(6))} USD
                <span
                  className={
                    currencyData.quotes.USD.percent_change_24h > 0
                      ? 'positive'
                      : 'negative'
                  }
                >
              ...
              <Btc>{currencyData.quotes.BTC.price} BTC</Btc>
            </HeadRates>
            <HeadLogo>
              <img src={logo} />
            </HeadLogo>
          ...
          <Body>
            <BodyLeftColumn>
              <BodyTitle>{trans('currencyWidget.rank')}</BodyTitle>
              <BodyValue>{currencyData.rank}</BodyValue>
            </BodyLeftColumn>
            <BodyCenterColumn>
              <BodyTitle>{trans('currencyWidget.marketCap')}</BodyTitle>
              <BodyValue>
                ${currencyToSize(currencyData.quotes.USD.market_cap)}
              </BodyValue>
            </BodyCenterColumn>
            <BodyRightColumn>
              <BodyTitle>{trans('currencyWidget.volume24h')}</BodyTitle>
              <BodyValue>
          ...
          <Footer>
            <a href="#">{trans('currencyWidget.footerLink')}</a>
          </Footer>
        </Index>
      )
    );
