...
import BNBIcon from '../../../Wallet/components/BNBIcon';
...
class UnpuckTorrent extends PureComponent {
  ...
  handlerInputGas = (e, gasPrice, gasLimit) => {
    this.setState({
      gasPriceValid: !!gasPrice,
      gasPrice,
      gasLimitValid: !!gasLimit,
      gasLimit
    });
  };
  ...
  render() {
    ...
    const priceIncludedFee = torrent
      ? getETHFee(percentageFee, torrent.price)
          .plus(new BigNumber(torrent.price))
          .toString()
      : 0;
      ...
      <PopUp>
        ...
        <CostsWrapper>
          <Costs>
            <BNBIcon />
            <CostsValue>
              {priceIncludedFee} {trans('popups.upload.ETH')}
            </CostsValue>
          </Costs>
        </CostsWrapper>
        <RowCenter>
        ...
        <CancelBtn onClick={onCancelUnpackTorrent} title={trans('Cancel')} />
      </PopUp>
    );
  }
}
